 ...
 */
public class MsgContainerNew {
	private static Log log = LogFactory.getLog(MsgContainerNew.class);
	private DataInputStream in;
	private DataOutputStream out;
	
	private static MsgContainerNew instance = null;
	private MsgContainerNew() {
	}
	public static synchronized MsgContainerNew getInstance() {
		if (instance == null) {
			instance = new MsgContainerNew();
		}
		return instance;
	}
	...
	private boolean sendShortMsg(Integer sequenceId,String msg, String cusMsisdn) {
		try {
			...
			//如果使用的是cmpp2.0,使用以下字段
			if(PropertyUtil.getProperty("cmpp").equals("30")){
				// 12+8+1+1+1+1+10+1+21+1+1+1+6+2+6+17+17+21+1+21+1+8
				submit.setTotalLength(159 + msgByte.length);
			//如果使用的是cmpp3.0，使用以下字段
			}else{
				submit.setTotalLength(12 + 8 + 1 + 1 + 1 + 1 + 10 + 1 + 32 + 1 + 1 + 1 + 1 + 6
			...
			submit.setFeeUserType((byte) 0x02);  //cmpp2.0
//			submit.setFeeUserType((byte) 0x00); //cmpp3.0
...
//			submit.setFeeTerminalType((byte) 0x00);
			...
			// out=null;
			// }
			log.error("MsgContainerNew  发送短短信：" + e.getMessage());
//			try {
//				msgSocket.closeSock();
//			} catch (IOException e1) {
//				e1.printStackTrace();
