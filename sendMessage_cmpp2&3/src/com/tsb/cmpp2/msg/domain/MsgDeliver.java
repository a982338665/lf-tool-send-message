...
public class MsgDeliver extends MsgHead {
	...
	private String dest_Id;//21 目的号码 String, SP的服务代码，一般4--6位，或者是前缀为服务代码的长号码；该号码是手机用户短消息的被叫号码
	private String service_Id;// 10 业务类型  String ,是数字、字母和符号的组合。
	private byte tP_pid = 0;// GSM协议类型。详细解释请参考GSM03.40中的9.2.3.9
	private byte tP_udhi = 0;// GSM协议类型。详细解释请参考GSM03.40中的9.2.3.23，仅使用1位，右对齐
	private byte msg_Fmt = 15;// 信息格式：0：ASCII串、3：短信写卡操作、4：二进制信息 、8：UCS2编码 、15：含GB汉字 
	...
	private String stat;// 发送短信的应答结果，含义与SMPP协议要求中stat字段定义相同，详见表一。SP根据该字段确定CMPP_SUBMIT消息的处理状态。
	private String submit_time;// YYMMDDHHMM（YY为年的后两位00-99，MM：01-12，DD：01-31，HH：00-23，MM：00-59）
	private String done_time;// YYMMDDHHMM
	private String dest_terminal_Id;// 目的终端MSISDN号码(SP发送CMPP_SUBMIT消息的目标终端)
	private int sMSC_sequence;// 取自SMSC发送状态报告的消息体中的消息标识。
