...
public class CMPPMo3 extends Thread {
	/**
	 * Logger for this class
	 */
	private static final Log log = LogFactory.getLog(CMPPMo3.class);
	 ...
	 * cmpp调用
	 */
	private static CMPPService3 cmpp;
...
//	}
	...
	private SendMsgThread3 sendThread;
		...
		while (true) {
			try {
				...
				if(initialSock){
					...
					cmpp = new CMPPService3(socket, delayTime);
					// 建立和网关的connect连接
					status = cmpp.cmppConnect(spid, password,(byte) Integer.parseInt(MsgConfig.get("version")));
				}
				...
				if (status == 0) {
					...
					sendThread = new SendMsgThread3(socket);
...
//					beginTime = System.currentTimeMillis();
					...
					case CommandID.CMPP_DELIVER:
						log.info("revc deliver message/CMPP_DELIVER:--->"+ CommandID.CMPP_DELIVER);
						deliver(resppacket);
						break;
					case CommandID.CMPP_ACTIVE_TEST:
						rec_returnheartTime =System.currentTimeMillis();//服务器返回心跳时间
						log.info("revc cmppactive message/CommandID.CMPP_ACTIVE_TEST-->"+ CommandID.CMPP_ACTIVE_TEST);
						active(resppacket);
						break;
					case CommandID.CMPP_ACTIVE_TEST_RESP:
						rec_returnheartTime =System.currentTimeMillis();//服务器返回心跳时间
						log.info("revc cmppactive resp message/CMPP_ACTIVE_TEST_RESP-->"+ CommandID.CMPP_ACTIVE_TEST_RESP);
						break;
					case CommandID.CMPP_DELIVER_RESP:
						log.info("unresonable message/CommandID.CMPP_DELIVER_RESP-->"+ CommandID.CMPP_DELIVER_RESP);
						break;
					case CommandID.CMPP_SUBMIT_RESP:
						log.info("revc submitresp/CommandID.CMPP_SUBMIT_RESP-->"+ CommandID.CMPP_SUBMIT_RESP);
						CMPPSubmitResp submitResp = new CMPPSubmitResp();
						submitResp.parseResponseBody(resppacket);
						log.info("resppacket.length/submitResp.getTotalLength()-->"+resppacket.length+"/"+submitResp.getTotalLength());
						if (resppacket.length != submitResp.getTotalLength()) {
				            log.error("CMPP_SUBMIT_RESP 返回包长度不对");
				          }else {
//				            submitResp.parseResponseBody(resppacket);
				            log.info("submitResp.getSequenceID==-->"+submitResp.getSequenceID());
